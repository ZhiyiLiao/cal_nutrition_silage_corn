...
class FertilizerRecommend:
    ...
    def _get_interpolated_recommendation(self, rec_dict: dict, crop_type: str, target_yield: float,
        ...
        # 获取当前级别和上一级别（更高级别）的推荐量
        rec_lower = self._get_recommendation(rec_dict, crop_type, target_yield, level)       # 当前级别
        rec_upper = self._get_recommendation(rec_dict, crop_type, target_yield, level - 1)   # 上一级

        # 获取当前级别的下限和上限（即上一级别的下限）
        bounds = thr_dict[region]  # [级别7, 级别6, ..., 级别1]
        idx = 7 - level            # 当前级别在数组中的索引
        lower = bounds[idx]        # 当前级别下限
        upper = bounds[idx + 1]    # 上一级别下限（即当前级别的上限），因为索引+1对应更高级别

        # 若下限或上限无效，则退化为当前级别推荐量
        if lower is None or upper is None or not np.isfinite(lower) or not np.isfinite(upper) or upper <= lower:
            return rec_lower

        # 计算测土值在区间内的相对位置 ratio ∈ [0,1]
        ratio = (soil_val - lower) / (upper - lower)
        ratio = max(0, min(1, ratio))

        # 线性插值：当 ratio=0（接近下限）→ rec_lower；ratio=1（接近上限）→ rec_upper
...
if __name__ == "__main__":
    ...
    soil_k = 120
    target_yield = 30
    crop_type = "青贮"  # 或 "籽实" 等其他类型

    # 获取推荐（无需传递 adjust_factor）
