...
class FertilizerRecommend:
    ...
    def _adjust_by_soil(self, region, soil_val, level, base_recommend, factor, thr_dict):
        """根据测土值在级别内的相对位置调整推荐量"""
        lower_bounds = thr_dict[region]  # [级别7, 级别6, ..., 级别1]
        # 获取当前级别的下限
        lower_current = lower_bounds[7 - level]  # level=1 -> index6, level=7 -> index0
        if lower_current is None or np.isinf(lower_current):
            # 下限为无穷（级别7的下限可能是 -inf），此时无法计算，直接返回基础值
        ...
        if level == 1:
            ...
            lower_upper = lower_bounds[7 - (level - 1)]  # 上一级（更高级别）的下限
            if lower_upper is None or np.isinf(lower_upper):
                ratio = 0
            else:
                # 防止除零
                if lower_upper == lower_current:
                    ratio = 0
                else:
                    ratio = (soil_val - lower_current) / (lower_upper - lower_current)
                    # 限制在 [0,1] 范围内（由于浮点误差可能略超）
                    ratio = max(0, min(1, ratio))

        # 调整：测土值越高，推荐量越少（系数 1 - ratio * factor）
        adjust_coeff = 1 - ratio * factor
        return base_recommend * adjust_coeff
    def recommend(self, region: str, soil_n: float, soil_p: float, soil_k: float,
                  target_yield: float, crop_type: str = '籽实', adjust_factor=0.15) -> Tuple[float, float, float]:
        """
        返回 (N, P2O5, K2O) 推荐量，调整因子 adjust_factor 控制最大减少比例（默认15%）
        """
        # 1. 确定级别（原逻辑不变）
        ...
        # 2. 获取基础推荐量（原逻辑不变）
        n_base = self._get_recommendation(self.n_rec_dict, crop_type, target_yield, n_level)
        p_base = self._get_recommendation(self.p_rec_dict, crop_type, target_yield, p_level)
        k_base = self._get_recommendation(self.k_rec_dict, crop_type, target_yield, k_level)

        # 3. 计算每个元素的级别内位置系数并调整
        n_adj = self._adjust_by_soil(region, soil_n, n_level, n_base, adjust_factor, self.n_thr_dict)
        p_adj = self._adjust_by_soil(region, soil_p, p_level, p_base, adjust_factor, self.p_thr_dict)
        k_adj = self._adjust_by_soil(region, soil_k, k_level, k_base, adjust_factor, self.k_thr_dict)
