...
class FertilizerRecommend:
    ...
    def recommend(self, region: str, soil_n: float, soil_p: float, soil_k: float,
                  target_yield: float, crop_type: str = '籽实',
                  output_unit: str = 'kg/ha', target_unit: str = 't/ha') -> Tuple[float, float, float]:
        """
        主函数：返回 (N_recom, P2O5_recom, K2O_recom)

        参数:
            region: 地区
            soil_n, soil_p, soil_k: 测土值
            target_yield: 目标产量数值
            crop_type: '籽实' 或 '青贮'
            output_unit: 输出单位，可选 'kg/ha' (默认) 或 'kg/mu' (公斤/亩)
            target_unit: 目标产量输入单位，可选 't/ha' (吨/公顷，默认) 或 'kg/mu' (公斤/亩)
        """
        # 转换目标产量为插值所用的 t/ha
        if target_unit == 'kg/mu':
            # 1 t/ha = 1000 kg / 15亩 = 66.6667 kg/亩
            # 所以 kg/亩 -> t/ha: 除以 (1000/15) = 乘 0.015
            target_t_ha = target_yield * 15 / 1000.0
        elif target_unit == 't/ha':
            target_t_ha = target_yield
        else:
            raise ValueError("target_unit 必须是 't/ha' 或 'kg/mu'")
        ...
        n_rec = self._get_interpolated_recommendation(
            self.n_rec_dict, crop_type, target_t_ha, n_level, region, soil_n, self.n_thr_dict)
        p_rec = self._get_interpolated_recommendation(
            self.p_rec_dict, crop_type, target_t_ha, p_level, region, soil_p, self.p_thr_dict)
        k_rec = self._get_interpolated_recommendation(
            self.k_rec_dict, crop_type, target_t_ha, k_level, region, soil_k, self.k_thr_dict)

        # 转换输出单位
        if output_unit == 'kg/mu':
            n_rec /= 15
            p_rec /= 15
            k_rec /= 15
        elif output_unit != 'kg/ha':
            raise ValueError("output_unit 必须是 'kg/ha' 或 'kg/mu'")
...
if __name__ == "__main__":
    excel_file = "玉米施肥推荐_籽实和青贮.xlsx"
    fr = FertilizerRecommend(excel_file)

    # ---------- 青贮：目标产量输入 t/ha，输出 kg/ha ----------
    region = "吉林中部"
    soil_n, soil_p, soil_k = 72, 15, 120
    # target_yield = 60  # t/ha
    target_yield = 6000  # kg/mu
    n, p, k = fr.recommend(region, soil_n, soil_p, soil_k, target_yield,
                           crop_type="青贮", output_unit="kg/mu", target_unit="kg/mu")
    n_indicator, n_unit = fr.get_nitrogen_indicator(region)
    print("===== 青贮玉米 (输入t/ha, 输出kg/mu) =====")
    print(f"地区: {region}")
    print(f"测土{n_indicator}: {soil_n} {n_unit}, 测土速效磷: {soil_p} mg/kg, 测土有效钾: {soil_k} mg/kg")
    print(f"目标产量: {target_yield} kg/亩")
    print(f"推荐施肥量: N = {n:.2f} kg/亩, P₂O₅ = {p:.2f} kg/亩, K₂O = {k:.2f} kg/亩\n")

    # ---------- 籽实：目标产量输入 kg/亩，输出 kg/亩 ----------
    target_yield_kg_mu = 600  # kg/亩
    n, p, k = fr.recommend(region, soil_n, soil_p, soil_k, target_yield_kg_mu,
                           crop_type="籽实", output_unit="kg/mu", target_unit="kg/mu")
    print("===== 籽实玉米 (输入kg/亩, 输出kg/亩) =====")
    print(f"地区: {region}")
    print(f"测土{n_indicator}: {soil_n} {n_unit}, 测土速效磷: {soil_p} mg/kg, 测土有效钾: {soil_k} mg/kg")
    print(f"目标产量: {target_yield_kg_mu} kg/亩")
    print(f"推荐施肥量: N = {n:.2f} kg/亩, P₂O₅ = {p:.2f} kg/亩, K₂O = {k:.2f} kg/亩")